...
class WP_Site_Health {
	...
	public function get_test_wordpress_version() {
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'We were unable to check if any new versions of Chartí CMS are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of Chartí CMS' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of Chartí CMS is available.' )
						);
					} else {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							_n(
								'Your site has %1$d inactive theme, other than %2$s, the default theme, and %3$s, your active theme.',
								'Your site has %1$d inactive themes, other than %2$s, the default theme, and %3$s, your active theme.',
								$themes_inactive
							),
							$themes_inactive,
							$default_theme ? $default_theme->name : WP_DEFAULT_THEME,
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured on loading. This is necessary for correct calculations of dates and times.' )
			),
		...
		if ( 'UTC' !== date_default_timezone_get() ) {
			...
			$result['description'] = sprintf(
				...
				sprintf(
					...
					__( 'PHP default timezone was changed after loading by a %s function call. This interferes with correct calculations of dates and times.' ),
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The SQL server is a required piece of software for the database uses to store all your site&#8217;s content and settings.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener noreferrer">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
		...
		if ( ! $this->mysql_min_version_check ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'Requires %1$s version %2$s or higher. Contact your web hosting company to correct this.' ),
					( $this->is_mariadb ? 'MariaDB' : 'MySQL' ),
			...
			$result['description'] .= sprintf(
				...
				wp_kses(
					sprintf(
						...
						'<code>WP_CONTENT_DIR/db.php</code>',
						( $this->is_mariadb ? 'MariaDB' : 'MySQL' )
					),
					array(
						'code' => true,
	...
	public function get_test_utf8mb4_support() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'UTF8MB4 is the character set preferable for database storage because it safely supports the widest set of characters and encodings, including Emoji, enabling better support for non-English languages.' )
			),
			'actions'     => '',
		...
		if ( ! $this->is_mariadb ) {
			if ( version_compare( $this->mysql_server_version, '5.5.3', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( 'Chartí CMS&#8217; utf8mb4 support requires MySQL version %s or greater. Please contact your server administrator.' ),
						'5.5.3'
					)
				);
			} else {
			...
			if ( version_compare( $this->mysql_server_version, '5.5.0', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						/* translators: %s: Version number. */
						__( 'Chartí CMS&#8217; utf8mb4 support requires MariaDB version %s or greater. Please contact your server administrator.' ),
						'5.5.0'
					)
				);
			} else {
			...
			if ( version_compare( $mysql_client_version, '5.0.9', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( 'Chartí CMS&#8217; utf8mb4 support requires MySQL client library (%1$s) version %2$s or newer. Please contact your server administrator.' ),
						'mysqlnd',
						'5.0.9'
					)
				);
	...
	public function get_test_dotorg_communication() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Communicating with the WP Servers is used to check for new versions, and to both install and update themes or plugins.' )
			),
			'actions'     => '',
			...
			if ( 'https' !== substr( $wp_url, 0, 5 ) || 'https' !== substr( $site_url, 0, 5 ) ) {
				...
				$result['description'] = sprintf(
					...
					sprintf(
						...
						esc_url( admin_url( 'settings/index.php' ) )
					)
				);
	...
	public function get_test_background_updates() {
		...
		if ( ! class_exists( 'WP_Site_Health_Auto_Updates' ) ) {
			require_once ABSPATH . ADMIN_DIR . '/includes/class-wp-site-health-auto-updates.php';
		}
	...
	public static function get_tests() {
		 ...
		 * to complete should run asynchronously, to avoid extended loading periods within ADMIN_DIR.
		 *
		 * @since 5.2.0
		 *
	...
	public function wp_cron_scheduled_check() {
		// Bootstrap ADMIN_DIR, as WP_Cron doesn't do this for us.
		require_once trailingslashit( ABSPATH ) . ADMIN_DIR . '/includes/admin.php';
