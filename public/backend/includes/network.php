...
/**
 * Getting the admin path
 * @since Thirsty Core 0.1
*/
	$admin_dir_location = ADMIN_DIR;
	$includes_dir_location = WPINC;
	$content_dir_location = WP_CONTENT_DIR;
...
function network_step1( $errors = false ) {
	...
	if ( WP_CONTENT_DIR !== ABSPATH . WP_CONTENT_DIR && ( allow_subdirectory_install() || ! allow_subdomain_install() ) ) {
		echo '<div class="error inline"><p><strong>' . __( 'Warning:' ) . '</strong> ' . __( 'Subdirectory networks may not be fully compatible with custom ' . WP_CONTENT_DIR . ' directories.' ) . '</p></div>';
	}
...
function network_step2( $errors = false ) {
	...
	<ol>
		...
		<?php
		printf(
			/* translators: 1: configuration.php, 2: Location of configuration file, 3: Translated version of "That's all, stop editing! Happy publishing." */
			__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ),
			'<code>configuration.php</code>',
			'<code>' . $location_of_wp_config . '</code>',
			/*
			 * translators: This string should only be translated if configuration-sample.php is localized.
			 * You can check the localized release package or
			 * https://i18n.svn.wordpress.org/<locale code>/branches/<wp version>/dist/configuration-sample.php
			 */
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			...
			$web_config_file .= '
                <rule name="WordPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . $admin_dir_location . '$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . $admin_dir_location . '/" redirectType="Permanent" />
                </rule>
...
';
		...
		echo '</p>';
		if ( ! $subdomain_install && WP_CONTENT_DIR !== ABSPATH . WP_CONTENT_DIR ) {
			echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom ' . WP_CONTENT_DIR .' directories.' ) . '</strong></p>';
		}
		?>
...
RewriteRule ^{$subdir_match}{$admin_dir_location}$ {$subdir_replacement_01}{$admin_dir_location}/ [R=301,L]

RewriteCond %{REQUEST_FILENAME} -f [OR]
RewriteCond %{REQUEST_FILENAME} -d
RewriteRule ^ - [L]
RewriteRule ^{$subdir_match}(({$content_dir_location}|{$admin_dir_location}|{$includes_dir_location}).*) {$rewrite_base}{$subdir_replacement_12} [L]
