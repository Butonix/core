...
require_once ABSPATH . ADMIN_DIR . '/includes/admin.php';

/** WordPress Schema API */
require_once ABSPATH . ADMIN_DIR . '/includes/schema.php';
	...
	function wp_install_defaults( $user_id ) {
		...
		if ( is_multisite() ) {
			...
			if ( ! $first_post ) {
				...
				__( 'Hi. This is your first post using Chartí CMS. Edit or delete it, then start' ) .
				"</p>\n<!-- /wp:paragraph -->";
			}
			...
			$first_post = "<!-- wp:paragraph -->\n<p>" .
			/* translators: First post content. %s: Site link. */
			__( 'Hi. This is your first post using Chartí CMS. Edit or delete it, then start writing!' ) .
			"</p>\n<!-- /wp:paragraph -->";
		}
		...
		$wpdb->insert(
			...
			array(
				...
				'post_excerpt'          => '',
				'post_title'            => __( 'This is Chartí' ),
				/* translators: Default post slug. */
				'post_name'             => sanitize_title( _x( 'this-is-charti', 'Default post slug' ) ),
				'post_modified'         => $now,
	...
	foreach ( $files as $oldfile => $newfile ) {
		...
		if ( $lines ) {
			...
			foreach ( $lines as $line ) {
				if ( preg_match( '/require.*autoload/', $line ) ) {
					$line = '//' . $line;
				}
