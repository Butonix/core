...
function _unzip_file_ziparchive( $file, $to, $needed_dirs = array() ) {
	 ...
	 */
	if ( wp_doing_cron() ) {
		$available_space = @disk_free_space( ABSPATH . WP_CONTENT_DIR );
...
function _unzip_file_pclzip( $file, $to, $needed_dirs = array() ) {
	...
	require_once ABSPATH . ADMIN_DIR . '/includes/class-pclzip.php';
...
function WP_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	...
	require_once ABSPATH . ADMIN_DIR . '/includes/class-wp-filesystem-base.php';
	...
	if ( ! class_exists( "WP_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . ADMIN_DIR . '/includes/class-wp-filesystem-' . $method . '.php', $method );
 ...
 *
 * @link https://wordpress.org/support/article/editing-configuration-php/#wordpress-upgrade-constants
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = ABSPATH . WP_CONTENT_DIR;
	}

	// If the directory doesn't exist (ABSPATH . WP_CONTENT_DIR/languages) then use the parent directory as we'll create it.
