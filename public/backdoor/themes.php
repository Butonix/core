...
// to be moved on /themes/class/themes.php
class Themes extends Settings_page {

	public function __construct() {
    // Check if the current user has enough permissions to manage this page
    parent::permission_handler('edit_theme_options');
	...
	protected function theme_handler() {
		if ( 'activate' == $_GET['action'] ) {
			$this->theme_activation_handler();
		} elseif ( 'resume' === $_GET['action'] ) {
			$this->theme_resume_handler();
		} elseif ( 'delete' == $_GET['action'] ) {

		}

	}
	protected function theme_activation_handler() {
		// check the nonce
		check_admin_referer( 'switch-theme_' . $_GET['stylesheet'] );
		// any errors while activation ?
		$this->error_while_activation();
		// switch theme
		$this->theme_switcher();
	}

	protected function theme_resume_handler() {
		// check the nonce
		check_admin_referer( 'resume-theme_' . $_GET['stylesheet'] );
		// to be moved to a protected function for permissions so we can reuse this as a handler
		...
		$result = resume_theme( $this->get_theme()->get_stylesheet(), self_admin_url( 'themes.php?error=resuming' ) );

		if ( is_wp_error( $result ) ) {
			wp_die( $result );
		}

		$this->redirect_handler( 'themes.php?resumed=true' );
	}
	...
	// check if any error while activation
	protected function error_while_activation() {
		
		if(! $this->get_theme()->exists() || ! $this->get_theme()->is_allowed()) {
			wp_die(
	...
	protected function theme_switcher() {
		// switch the current theme with the selected one
		switch_theme( $this->get_theme()->get_stylesheet() );
		// prompt the activated message
		$this->redirect_handler('themes.php?activated=true');
	}

	protected function redirect_handler($redirect_to) {
...
// to be moved on /themes/views/themes.php
...
foreach ( $themes as $theme ) :
	...
	//var_dump($theme['screenshot']); die;s
