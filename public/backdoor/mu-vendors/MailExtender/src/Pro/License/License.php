...
class License {
	...
	public function display_settings_license_key_field_content( $options ) {
		...
		<!-- <button id="wp-mail-smtp-setting-license-key-verify" class="wp-mail-smtp-btn wp-mail-smtp-btn-md wp-mail-smtp-btn-orange">
...
wp-mail-smtp-hide' : '';
	...
	public function validate_key( $key = '', $forced = false, $ajax = false ) {
		...
		if ( isset( $validate->expired ) ) {
			...
			$options->set( $all_opt );
			if ( $ajax ) {
				wp_send_json_error( esc_html__( 'Your license key for WP Mail SMTP Pro has expired. Please renew your license key on WPForms.com to continue receiving automatic updates.', 'wp-mail-smtp-pro' ) );
			}
