...
namespace Core\Ethos\MetaBoxes;
...
class Metabox {
	...
	public function scripts() {
		...
		if ( in_array( $screen->id, $this->get_post_type() ) ) {
			...
			// Metabox.
			wp_enqueue_script( 'Core-admin', plugins_url( '../../assets/js/admin.js', __FILE__ ), array( 'jquery' ), null, true );
			wp_enqueue_style( 'Core-admin', plugins_url( '../../assets/css/admin.css', __FILE__ ), array(), null, 'all' );
			...
			wp_localize_script(
				...
				'CoreAdminParams',
				array(
					'galleryTitle'  => __( 'Add images in gallery', 'Core' ),
					'galleryButton' => __( 'Add in gallery', 'Core' ),
					'galleryRemove' => __( 'Remove image', 'Core' ),
					'uploadTitle'   => __( 'Choose a file', 'Core' ),
					'uploadButton'  => __( 'Add file', 'Core' ),
				)
			);
		}
	}
	...
	public function metabox( $post ) {
		...
		do_action( 'Core_metabox_header_' . $this->id, $post_id );

		echo apply_filters( 'Core_metabox_container_before_' . $this->id, '<table class="form-table Core-form-table">' );

		foreach ( $this->fields as $field ) {
			echo apply_filters( 'Core_metabox_wrap_before_' . $this->id, '<tr valign="top">', $field );

			if ( 'title' == $field['type'] ) {
				$title = sprintf( '<th colspan="2"><strong>%s</strong></th>', $field['label'] );
			} elseif ( 'separator' == $field['type'] ) {
				$title = sprintf( '<td colspan="2"><span id="Core-metabox-separator-%s" class="Core-metabox-separator"></span></td>', $field['id'] );
			} else {
				$title = sprintf( '<th><label for="%s">%s</label></th>', $field['id'], $field['label'] );
			}

			echo apply_filters( 'Core_metabox_field_title_' . $this->id, $title, $field );

			echo apply_filters( 'Core_metabox_field_before_' . $field['id'], '<td>', $field );
			...
			echo apply_filters( 'Core_metabox_field_after_' . $field['id'], '</td>', $field );

			echo apply_filters( 'Core_metabox_wrap_after_' . $this->id, '</tr>', $field );
		}

		echo apply_filters( 'Core_metabox_container_after_' . $this->id, '</table>' );

		do_action( 'Core_metabox_footer_' . $this->id, $post_id );
	...
	protected function process_fields( $args, $post_id ) {
		...
		switch ( $type ) {
			...
			case 'editor':
				...
				break;
			case 'color':
				$this->field_input( $id, $current, array_merge( array( 'class' => 'Core-color-field' ), $attrs ) );
				break;
			case 'upload':
			...
			default:
				do_action( 'Core_metabox_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button Core-upload-button" type="button" value="%3$s" />', $id, esc_url( $current ), __( 'Select file', 'Core' ), $this->build_field_attributes( $attrs ) );
	}
	...
	protected function field_image( $id, $current ) {
		...
		$image = apply_filters( 'Core_placeholder_path', plugins_url( '../../assets/images/placeholder.png', __FILE__ ) );
		$html  = '<div class="Core-upload-image">';
	...
	protected function field_image_plupload( $id, $current ) {
		$html = '<div class="Core-gallery-container">';
			$html .= '<ul class="Core-gallery-images">';
				if ( ! empty( $current ) ) {
			...
			$html .= sprintf( '<input type="hidden" class="Core-gallery-field" name="%s" value="%s" />', $id, $current );

			// Adds "adds images in gallery" url.
			$html .= sprintf( '<p class="Core-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'Core' ) );
		$html .= '</div>';
	...
	public function save( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( ! in_array( $field['type'], array( 'separator', 'title' ) ) ) {
				...
				$new = apply_filters( 'Core_save_metabox_' . $this->id, $value, $name );
