...
class WP_Fatal_Error_Handler {
	...
	protected function display_default_error_template( $error, $handled ) {
		...
		if ( ! class_exists( 'WP_Error' ) ) {
			require_once ABSPATH . WPINC . '/classes/class-wp-error.php';
		}
		...
		$message = sprintf(
			...
			$message,
			/* translators: Documentation explaining debugging in Thirsty Core. */
			esc_url( __( 'https://wordpress.org/support/article/debugging-in-wordpress/' ) ),
			__( 'Learn more about debugging in Thirsty Core.' )
		);
	...
	/**
	 * Fancy errors for devs
	 * @since Thirsty Core 0.1
	 */

		//wp_die( $wp_error, '', $args );

		echo '<style>* {margin:0; padding:0} body {background:beige; color:#222; padding:25px; font-size:14px; line-height:1.2rem} a { color:blue}</style><pre>';
		var_dump($wp_error);
		echo '</pre>';
	}
}
