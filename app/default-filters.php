...
// Display URL.
foreach ( array( 'user_url', 'link_url', 'link_image', 'comment_url', 'post_guid' ) as $filter ) {
	if ( is_admin() ) {
		add_filter( $filter, 'wp_strip_all_tags' );
	}
...
add_action( 'wp_head', 'wp_resource_hints', 2 ); // TOODCHARTI
...
// add_action( 'wp_head', 'wp_shortlink_wp_head', 10, 0 ); TOODCHARTI
...
// Charti CMS
add_action( 'admin_enqueue_scripts', 'v2admin_tinymce_scripts' );
add_action( 'admin_enqueue_scripts', 'flexdatalist' );
//
...
/**
 * Thirsty Core
 * We are going to disable the canical redirect for alternative paths
 * @since 0.1
**/
