...
function do_enclose( $content = null, $post ) {
	...
	include_once ABSPATH . WPINC . '/classes/class-IXR.php';
...
function do_robots() {
	...
	$output  .= "Disallow: $path .'/'" . ADMIN_DIR ."'./'\n";
	$output  .= "Allow: $path .'/'" . ADMIN_DIR . "'/'admin-ajax.php\n";
 ...
 * It may be overridden by the WP_TEMP_DIR constant in your configuration.php file.
 *
 * @since 2.5.0
 *
...
function _wp_upload_dir( $time = null ) {
	...
	if ( empty( $upload_path ) || WP_CONTENT_DIR . '/uploads' == $upload_path ) {
	...
	if ( ! $url ) {
		if ( empty( $upload_path ) || ( WP_CONTENT_DIR . '/uploads' == $upload_path ) || ( $upload_path == $dir ) ) {
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had WP_CONTENT_DIR/uploads for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as WP_CONTENT_DIR/uploads is used
			 *    there, and
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	/** This filter is documented in ADMIN_DIR/includes/file.php */
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="<?php echo wp_guess_url() . '/' . ADMIN_DIR . '/css/install.css'; ?>">
	<link rel="stylesheet" type="text/css" href="<?php echo wp_guess_url() . '/' . WPINC . '/css/buttons.css'; ?>">
	<style type="text/css">
		#error-page {
			margin-top: 50px;
		}
...
<body id="error-page" class="wp-core-ui">
 ...
 *
 * If a file exists in the WP_CONTENT_DIR directory named db-error.php, then it will
 ...
 *
 * Will remove ADMIN_DIR links to retrieve only return URLs not in the ADMIN_DIR
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined( 'WP_SITEURL' ) && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], ADMIN_DIR ) !== false || strpos( $_SERVER['REQUEST_URI'], 'wp-login.php' ) !== false ) {
			$path = preg_replace( '#/(' . ADMIN_DIR .'/.*|wp-login.php)#i', '', $_SERVER['REQUEST_URI'] );
 ...
 *
 * The export files are stored in `WP_CONTENT_DIR/uploads`, and are therefore publicly
