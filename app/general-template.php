 ...
 *
 *
 * Extended by Charti CMS
 *
 * We are going to extend the old schoold get_header by adding more useful parameters
 * 
 * @since 0.1 
 *
 *
...
function get_header(
		$name = null, 			// Default WP @param used for a specific header-$name.php
		$page_id = null,		// post/page 'ID' or 'route'
		$action = null,			// boolean ( e.g is_user_logged_in() )
		$expected = null,			// expected context based on $action (true/false)
		$redirect_to = null 	// the url where visitors/users should be redirected
) {

	// Redirect to a specific page based on specific criterias
  if( $redirect_to && $page_id == 'route') {
  	if($action === $expected) {
  ...
  } //@TODOCHARTI to be extended for post_types
 ...
 *  Charti CMS
 *  @since 0.1
 *  @param string $side Expecting for "frontend" or "backend" Default: Null > Will look for frontend.
 */
function get_template_part( $slug, $name = null, $side = null, $directory = null) {
  /**
  ...
  locate_template( $templates, true, false, $side, $directory );
}
...
function wp_lostpassword_url( $redirect = '' ) {
  ...
  $lostpassword_url = add_query_arg( $args, network_site_url( 'login.php?action=lostpassword', 'login' ) );
 ...
 * 
 * Charti 
 * @see https://www.kirstencassidy.com/understanding-using-resource-hinting/
 * 
 * @since 0.1
 */
