	...
	if ( is_array( $wp_theme_directories ) && count( $wp_theme_directories ) > 1 ) {
		...
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			...
			if ( ! in_array( $root_of_current_theme, $wp_theme_directories ) ) {
				$root_of_current_theme = ABSPATH . '../' . WP_CONTENT_DIR . $root_of_current_theme;
			}
 ...
 * 
 * Modified by Chartí CMS
 * The directory theme root is located now outside from public directory.
 * 
 * @since 0.1
 */
function wp_get_theme( $stylesheet = '', $theme_root = '' ) {
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = ABSPATH . '../' . WP_CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $wp_theme_directories ) ) {
			$theme_root = ABSPATH . '../' . WP_CONTENT_DIR . $theme_root;
		}
	}
 ...
 * 
 * Chartí CMS
 * Will not touch this function since this is used for returning stylesheet directory & assets path
 * @since 0.1
 */
 ...
 * @return string
 *
 * This was modified by Chartí CMS
 * The directory path is located now outside from public directory. It is what it is
 * @since 0.1
 */
function get_stylesheet_directory_uri() {
	...
	$stylesheet_dir_uri = "$theme_root_uri/"; //changed by charti
...
function get_template_directory_uri() {
	...
	$template_dir_uri = "$theme_root_uri/"; // made by charti
...
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = ABSPATH . '../' . WP_CONTENT_DIR . '/' . $directory;
...
function search_theme_directories( $force = false ) {
	 ...
	 *
	 * This was modified by Chartí CMS
	 * The directory theme is located now outside from public directory.
	 * 
	 * @since 0.1
	 *
	 */
	foreach ( $wp_theme_directories as $theme_root ) {
		if ( 0 === strpos( $theme_root, ABSPATH . '../' . WP_CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( ABSPATH . '../' . WP_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	...
	foreach ( $wp_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				// ABSPATH . '../' . WP_CONTENT_DIR/themes/a-single-theme
				// ABSPATH . '../' . WP_CONTENT_DIR/themes is $theme_root, a-single-theme is $dir.
				...
				$found_theme = false;
				// ABSPATH . '../' . WP_CONTENT_DIR/themes/a-folder-of-themes/*
				// ABSPATH . '../' . WP_CONTENT_DIR/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs.
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $wp_theme_directories ) ) {
			...
			if ( 0 === strpos( $theme_root, ABSPATH . '../' . WP_CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( ABSPATH . '../' . WP_CONTENT_DIR, '', $theme_root ) );
			...
			$theme_root_uri = content_url( 'dist' ); //changed by charti
		}
	} else {
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( is_admin() ) {
			require_once ABSPATH . ADMIN_DIR . '/includes/class-custom-image-header.php';
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once ABSPATH . ADMIN_DIR . '/includes/class-custom-background.php';
...
function _wp_customize_include() {
	...
	require_once ABSPATH . WPINC . '/classes/class-wp-customize-manager.php';
