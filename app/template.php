 ...
 *	Charti CMS
 *	@since 0.1
 *	@param string			 $side looks for 'frontend' or 'backend'. Default null. Will look for default location in the current theme
 *
 * @return string The template filename if one is located.
 */
function locate_template( $template_names, $load = false, $require_once = true, $side = null, $directory = null ) {
	$located = '';

	// Check if should look for a frontend template
	if($side == 'frontend' || $side == null) {
		$stylesheet_path = STYLESHEETPATH;
	}
	// Check if should look for a backend template 
	if($side == 'backend') {
		$stylesheet_path = ADMIN_STYLESHEETPATH . '/../../backdoor/views/' . $directory;
	}
