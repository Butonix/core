...
function wp_user_settings() {
	...
	if ( isset( $_COOKIE[ 'settings-' . $user_id ] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE[ 'settings-' . $user_id ] );
		...
		$current    = isset( $_COOKIE[ 'settings-time-' . $user_id ] ) ? preg_replace( '/[^0-9]/', '', $_COOKIE[ 'settings-time-' . $user_id ] ) : 0;
	...
	setcookie( 'settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, null, $secure );
	setcookie( 'settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, null, $secure );
	$_COOKIE[ 'settings-' . $user_id ] = $settings;
}
...
function delete_all_user_settings() {
	...
	setcookie( 'settings-' . $user_id, ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
}
