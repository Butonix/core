...
function wp_check_php_mysql_versions() {
	...
	if ( ! extension_loaded( 'mysql' ) && ! extension_loaded( 'mysqli' ) && ! extension_loaded( 'mysqlnd' ) && ! file_exists( ABSPATH . WP_CONTENT_DIR . '/db.php' ) ) {
...
function wp_maintenance() {
	...
	if ( file_exists( ABSPATH . WP_CONTENT_DIR . '/maintenance.php' ) ) {
		require_once ABSPATH . WP_CONTENT_DIR . '/maintenance.php';
		die();
	}
 ...
 * All three can be defined in configuration.php. By default, `WP_DEBUG` and
 ...
 *
 * When `WP_DEBUG_LOG` is true, errors will be logged to `WP_CONTENT_DIR/debug.log`.
...
function wp_debug_mode() {
	...
	if ( WP_DEBUG ) {
		...
		if ( in_array( strtolower( (string) WP_DEBUG_LOG ), array( 'true', '1' ), true ) ) {
			$log_path = ABSPATH . WP_CONTENT_DIR . '/debug.log';
...
function wp_set_lang_dir() {
	if ( ! defined( 'WP_LANG_DIR' ) ) {
		if ( file_exists( ABSPATH . WP_CONTENT_DIR . '/languages' ) && @is_dir( ABSPATH . WP_CONTENT_DIR . '/languages' ) || ! @is_dir( ABSPATH . WPINC . '/languages' ) ) {
			/**
			 ...
			 * @since 2.1.0
			 */
			define( 'WP_LANG_DIR', ABSPATH . WP_CONTENT_DIR . '/languages' );
			if ( ! defined( 'LANGDIR' ) ) {
				// Old static relative path maintained for limited backward compatibility - won't work in some cases.
				define( 'LANGDIR', ABSPATH . WP_CONTENT_DIR . '/languages' );
			}
		} else {
			/**
			 * Server path of the language directory.
 ...
 *
 * If an object-cache.php file exists in the WP_CONTENT_DIR directory,
...
function wp_not_installed() {
	if ( is_multisite() ) {
		...
		$link = wp_guess_url() . '/' . ADMIN_DIR . '/install.php';
 ...
 *
 * The default directory is WP_CONTENT_DIR/mu-plugins. To change the default
 ...
 *
 * The default directory is `WP_CONTENT_DIR/plugins`. To change the default
...
function is_admin() {
	if ( isset( $GLOBALS['current_screen'] ) ) {
		// echo '<pre>';
		// var_dump($GLOBALS['current_screen']);
		// echo '</pre>';
...
function wp_load_translations_early() {
	...
	while ( true ) {
		...
		if ( defined( 'WP_LANG_DIR' ) && @is_dir( ABSPATH . WP_LANG_DIR ) ) {
			$locations[] = WP_LANG_DIR;
		}

		if ( defined( 'WP_CONTENT_DIR' ) && @is_dir( ABSPATH . WP_CONTENT_DIR . '/languages' ) ) {
			$locations[] = ABSPATH . WP_CONTENT_DIR . '/languages';
		}

		if ( @is_dir( ABSPATH . WP_CONTENT_DIR . '/languages' ) ) {
			$locations[] = ABSPATH . WP_CONTENT_DIR . '/languages';
		}
...
function wp_is_jsonp_request() {
	...
	/** This filter is documented in WPINC/rest-api/class-wp-rest-server.php */
